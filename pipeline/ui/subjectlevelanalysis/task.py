...
from ...io import get_condition_files, parse_condition_files
from ...utils import splitext
from .setting import PreSmoothingSettingStep


def _get_conditions(ctx):
    eventfile_dict = get_condition_files(ctx.spec.analyses[-1], ctx.database)
    ...
    bold_filepaths, conditions_list, onsets_list, durations_list = zip(
        *parse_condition_files(eventfile_dict, ctx.database)
    )
...
class ContrastValuesStep(Step):
    ...
    def run(self, ctx):
        ...
        if valuedict is None:
            return False
        newdict = {
            varname: valuedict[dsp] for dsp, varname in self.varname_by_str.items()
        }
    ...
    def run(self, ctx):
        ...
        while True:
            ...
            try:
                ...
                tags_dict = BoldTagsSchema().dump(analysis_obj.tags)
...
class TxtEventsStep(EventsStep):
    allowed_entities = ["subject", "session", "run", "task", "direction", "condition"]
