...
from ..compiler import scan_entities
...
class HaveContrastsStep(YesNoStep):
    ...
    def setup(self, ctx):
        ...
        self._append_view(
            TextView("Specify additional contrasts for categorical variables?")
        )
...
class FdGt0_5FilterStep(BaseCutoffFilterStep):
    header_str = "Specify the maximum allowed proportion of frames with (FramewiseDisplacement > 0.5)"
...
class InterceptOnlyGroupLevelAnalysisMotionFilterStep(
    GroupLevelAnalysisMotionFilterStep
):
            ...
            if is_valid:
                for variable, checked in zip(self.variables, filterdictlist):
                    if not all(checked.values()):
                        levels = [
                            k for k, is_selected in checked.items() if is_selected
                        ]
...
class SpreadsheetColumnTypeStep(Step):
    ...
    def setup(self, ctx):
        ...
        self.is_missing = True
        already_used = set(
            variable.name for variable in ctx.spec.analyses[-1].variables
        )
        ...
        if self.is_missing:
            ...
            for variable_str, value in valuedict.items():
                ...
                if vartype == "categorical":
                    varobj.levels = self.df[varname].unique().tolist()
...
class SpreadsheetStep(Step):
    ...
    def setup(self, ctx):
        self._append_view(
            TextView("Specify the covariates/group data spreadsheet file")
        )
        self.message = None
            ...
            try:
                ...
                for analysis_obj in ctx.spec.analyses:
                    if (
                        ...
                        ctx.spec.analyses[-1].variables = deepcopy(
                            analysis_obj.variables
                        )
                        break
                if ctx.spec.analyses[-1].variables is None:
                    ctx.spec.analyses[-1].variables = []
...
class GroupLevelAnalysisAggregateStep(Step):
    ...
    def _resolve_tags(self, ctx):
        ...
        subjectlevel_analysis_objs = []
        for obj in ctx.spec.analyses:
            if obj.name in input:
                if obj.level == "subject":
                    subjectlevel_analysis_objs.append(obj)

        entities_by_analysis = {
            analysis_obj.name: set() for analysis_obj in subjectlevel_analysis_objs
        }
        for analysis_obj in subjectlevel_analysis_objs:
            ...
            for entity in scan_entities:
                entityset = ctx.database.get_tags_set(filepaths, entity)
                if entityset is not None and len(entityset) > 1:
    ...
    def _get_fixed_effects_aggregate(self, ctx, name, across):
        ...
        # need to create
        basename = make_name_suggestion("aggregate", name, "across", p.plural(across))
    ...
    def setup(self, ctx):
        ...
        self.options = [
            p.inflect(f"Aggregate across plural('{entity}')")
        ...
        if len(self.options) > 0:
            ...
            self.input_view = MultipleChoiceInputView(
                self.options, checked=self.options
            )
            self._append_view(self.input_view)
            self._append_view(SpacerView(1))
