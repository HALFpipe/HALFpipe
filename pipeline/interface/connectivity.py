...
class ConnectivityMeasureInputSpec(BaseInterfaceInputSpec):
    in_file = traits.File(
        desc="Image file(s) from where to extract the data",
    ...
    atlas_type = traits.Enum(
        ...
        desc="The type of atlas", default="labels")

    standardize = traits.Either(
        False, traits.Enum("zscore", "psc"),
        desc="Strategy to standardize the signal", default=False)
    resampling_target = traits.Either(
        None, traits.Enum("data", "labels"),
        desc="Gives which image gives the final shape/size", default=None)
...
class ConnectivityMeasureOutputSpec(TraitedSpec):
    connectivity = traits.File(
        desc="Numpy text file with the connectivity matrix")
    timeseries = traits.File(
        desc="Numpy text file with the timeseries matrix")
...
class ConnectivityMeasure(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        conn_measure = nilearn.connectome.ConnectivityMeasure(
