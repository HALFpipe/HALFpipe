...
from ...utils import _ravel, _get_first
...
def init_glm_wf(metadata, conditions, name="glm"):
    """
    ...
    inputnode = pe.Node(
        niu.IdentityInterface(fields=["bold_file", "mask_file", "confounds"]),
        name="inputnode",
    )
    ...
    selectcolumns, confounds_column_names = make_confounds_selectcolumns(metadata)
    ...
    modelspec = pe.Node(
        interface=model.SpecifyModel(
            ...
            subject_info=Bunch(conditions=names, onsets=onsets, durations=durations),
        ),
        name="modelspec",
    )
    ...
    contrasts_ = [
        [k, "T"] + [list(i) for i in zip(*[(n, val) for n, val in v.items()])]
    ...
    maskcopes = pe.MapNode(
        interface=fsl.ApplyMask(), name="maskcopes", iterfield=["in_file"]
    )
    maskvarcopes = pe.MapNode(
        interface=fsl.ApplyMask(), name="maskvarcopes", iterfield=["in_file"]
    )
    maskzstats = pe.MapNode(
        interface=fsl.ApplyMask(), name="maskzstats", iterfield=["in_file"]
    )

    # split regression outputs by name
    splitcopes = pe.Node(
        interface=niu.Split(splits=[1 for conname in connames]), name="splitcopes"
    )
    splitvarcopes = pe.Node(
        interface=niu.Split(splits=[1 for conname in connames]), name="splitvarcopes"
    )
    splitzstats = pe.Node(
        interface=niu.Split(splits=[1 for conname in connames]), name="splitzstats"
    )
    ...
    if has_confounds:
        workflow.connect(
            [
                ...
                (selectcolumns, modelspec, [("out_file", "realignment_parameters")]),
            ]
        )

    workflow.connect(
        [
            (inputnode, modelspec, [("bold_file", "functional_runs")]),
            ...
            (stats, modelestimate, [(("out_stat", _get_float), "threshold")]),
            (
                modelgen,
                modelestimate,
                [("design_file", "design_file"), ("con_file", "tcon_file")],
            ),
            (inputnode, maskcopes, [("mask_file", "mask_file")]),
            (inputnode, maskvarcopes, [("mask_file", "mask_file")]),
            (inputnode, maskzstats, [("mask_file", "mask_file")]),
            (modelestimate, maskcopes, [(("copes", _ravel), "in_file"),]),
            (modelestimate, maskvarcopes, [(("varcopes", _ravel), "in_file"),]),
            (modelestimate, maskzstats, [(("zstats", _ravel), "in_file"),]),
            (maskcopes, splitcopes, [("out_file", "inlist"),]),
            (maskvarcopes, splitvarcopes, [("out_file", "inlist"),]),
            (maskzstats, splitzstats, [("out_file", "inlist"),]),
        ]
    )
    ...
    for i, conname in enumerate(connames):
        workflow.connect(
            [
                ...
                (modelestimate, outputnode, [("dof_file", "%s_dof_file" % conname)]),
            ]
        )
