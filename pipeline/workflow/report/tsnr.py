...
def init_tsnr_wf(name="tsnr", memcalc=MemoryCalculator()):
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=["report_file"]), name="outputnode"
    )

    # actually calculate the tsnr image
    tsnr = pe.Node(
        interface=nac.TSNR(), name="compute_tsnr", mem_gb=memcalc.series_std_gb
    )
    ...
    workflow.connect(
        [
            (inputnode, tsnr, [("bold_file", "in_file")]),
