...
def init_dualregression_wf(
    metadata, componentsfile, name="dualregression", memcalc=MemoryCalculator()
):
    """
    ...
    maths = pe.Node(
        interface=fsl.ApplyMask(), name="maths", mem_gb=memcalc.volume_std_gb
    )
    ...
    varnames = sum(
        [
            [
                "%s_stat" % componentname,
                "%s_var" % componentname,
                "%s_zstat" % componentname,
                "%s_dof_file" % componentname,
            ]
    ...
    workflow.connect(
        [
            ...
            (maths, glm0, [("out_file", "design"),]),
            (inputnode, glm0, [("bold_file", "in_file"), ("mask_file", "mask")]),
            (inputnode, glm1, [("bold_file", "in_file"), ("mask_file", "mask")]),
            (contrasttotsv, glm1, [("out_file", "contrasts")]),
            (glm1, splitcopesimage, [("out_cope", "in_file"),]),
            (glm1, splitvarcopesimage, [("out_varcb", "in_file"),]),
            (glm1, splitzstatsimage, [("out_z", "in_file"),]),
            (splitcopesimage, splitcopes, [("out_files", "inlist"),]),
            (splitvarcopesimage, splitvarcopes, [("out_files", "inlist"),]),
            (splitzstatsimage, splitzstats, [("out_files", "inlist"),]),
            (inputnode, gendoffile, [("bold_file", "in_file"),]),
            (gendoffile, outputnode, [("out_file", "dof_file"),]),
        ]
    )
    ...
    for i, componentname in enumerate(componentnames):
        workflow.connect(
            [
                ...
                (gendoffile, outputnode, [("out_file", "%s_dof_file" % componentname)]),
            ]
        )
