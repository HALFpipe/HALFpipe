...
def init_rest_wf(metadata, name="rest", memcalc=MemoryCalculator()):
    ...
    confoundsregressiond = init_confoundsregression_wf(
        metadata, name="confoundsregressiond"
    )
    if confoundsregressiond is not None:
        workflow.connect(
            *unfilteredFileEndpointd, confoundsregressiond, "inputnode.bold_file"
        )
        workflow.connect(
            [
                (
                    inputnode,
                    confoundsregressiond,
                    [
    ...
    workflow.connect(
        *confoundsFilteredFileEndpointd, bandpass_wf, "inputnode.bold_file"
    )
    bandpassFilteredFileEndpointd = [bandpass_wf, "outputnode.filtered_file"]

    workflow.connect(
        [(inputnode, bandpass_wf, [("mask_file", "inputnode.mask_file"),])]
    )
    ...
    if len(outnames) > 0:
        ...
        confoundsregressiondf = init_confoundsregression_wf(
            metadata, name="confoundsregressiondf"
        )
        if confoundsregressiondf is not None:
            workflow.connect(
                *unfilteredFileEndpointdf, confoundsregressiondf, "inputnode.bold_file"
            )
            workflow.connect(
                [
                    (
                        inputnode,
                        confoundsregressiondf,
                        [
                            ("confounds_d", "inputnode.confounds"),
                            ("mask_file", "inputnode.mask_file"),
                        ],
        ...
        workflow.connect(
            *confoundsFilteredFileEndpointdf, brainatlas_wf, "inputnode.bold_file"
        )
    ...
    if "ReHo" in metadata and metadata["ReHo"]:
        ...
        if len(outnames) > 0:
            workflow.connect(
                *bandpassFilteredFileEndpointd, reho_wf, "inputnode.bold_file"
            )
    ...
    if "ALFF" in metadata and metadata["ALFF"]:
        ...
        if len(outnames) > 0:
            workflow.connect(
                *confoundsFilteredFileEndpointd, alff_wf, "inputnode.bold_file"
            )
            workflow.connect(
                *bandpassFilteredFileEndpointd, alff_wf, "inputnode.filtered_file"
            )

    for workflowName, (wf, outnames, outfields) in outByWorkflowName.items():
        workflow.connect([(inputnode, wf, [("mask_file", "inputnode.mask_file")])])

    _, outfieldsByOutname = make_outputnode(workflow, outByWorkflowName)
