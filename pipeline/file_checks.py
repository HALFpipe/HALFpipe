...
def level_1_check(path_to_pipeline_json, workdir):
    ...
    report_name = (
        "report_"
        + os.path.splitext(os.path.basename(path_to_pipeline_json))[0]
        + ".txt"
    )
                ...
                if paradigm == "rest":
                    ...
                    if field in configuration["metadata"][paradigm]:
                        ...
                        for name in template_dict:
                            ...
                            command = (
                                "fslnvols "
                                + configuration["metadata"][paradigm][field][name]
                            )
                    ...
                    for field in fields:
                        if (
                            field in configuration["metadata"][paradigm]
                            and configuration["metadata"][paradigm][field]
                        ):
                            for suffix in suffixes:
                                file = field + suffix
                                my_list.append(file)
