...
class CutoffFilterSchema(Schema):
    ...
    action = fields.Str(
        dump_default="exclude", validate=validate.OneOf(["exclude"]), required=True
    )
...
class GroupFilterSchema(Schema):
    type = fields.Str(
        dump_default="group", validate=validate.Equal("group"), required=True
    )
...
class MissingFilterSchema(Schema):
    type = fields.Str(
        dump_default="missing", validate=validate.Equal("missing"), required=True
    )
    action = fields.Str(validate=validate.OneOf(["include", "exclude"]), required=True)
    variable = fields.Str(required=True)
