...
from nipype.interfaces.fsl import FEATModel, FILMGLS, Merge
import nipype.interfaces.utility as niu
...
def plot_fsl_design(design_file, png_name=None):
    ...
    for line in open(design_file):
        if line.strip() == '/Matrix': start = True; continue
        if start:
    ...
    ax.set_xlabel('Regressors'); ax.set_ylabel('Time points')
    ax.set_title('FSL Design Matrix'); fig.tight_layout()
...
def init_singletrials_wf(
    ...
    inputnode = pe.Node(
        niu.IdentityInterface(
            fields=[
                'tags', 'vals', 'metadata', 'bold', 'mask',
                'repetition_time', 'confounds_selected',
                'condition_names', 'condition_files', 'condition_units',
            ]
        ), name='inputnode'
    )
    ...
    merge_metadata_node = pe.Node(
        niu.Function(
            input_names=["existing_metadata", "new_metadata"],
    ...
    cond2lsa = pe.Node(
        niu.Function(
            input_names=['subject_info'],
            output_names=['lsa_subject_info'],
            function=cond_bunch_to_lsa
        ),
        name='cond2lsa'
    )
    ...
    merge_lsa = pe.Node(
        Merge(
            ...
            merged_file="merged_betas.nii.gz"
        ),
        name='merge_betas_lsa'
    )

    plot_lsa = pe.Node(
        niu.Function(
            input_names=['design_file','png_name'],
            output_names=['out_png'],
            function=plot_fsl_design
        ),
        name='plot_dm_lsa'
    )
    ...
    pick_lss = pe.Node(
        niu.Function(
            input_names=['param_lists','idx'],
            output_names=['roi_betas'],
            function=pick_roi_betas
        ),
        name='pick_lss_betas'
    )
    ...
    merge_resultdicts = pe.Node(
        niu.Merge(2),
        name="merge_resultdicts"
    )
    ...
    workflow.connect([
        ...
        # parse conditions
        (inputnode, parse_condition_file, [
            ('condition_names','condition_names'),
            ('condition_files','in_any')
        ]),
        ...
        (inputnode, make_resultdicts_lsa, [
            ('tags', 'tags'),
            ('vals', 'vals'),
            ('mask', 'mask'),
        ]),
        ...
        (inputnode, make_resultdicts_lss, [
            ('tags', 'tags'),
            ('vals', 'vals'),
            ('mask', 'mask'),
        ]),
