...
#             self.feature_dict["type"] = "atlas_based_connectivity"
...
#         if "atlases" not in self.feature_dict:
#             self.feature_dict["atlases"] = []
#
...
#             yield AtlasFilePanel(id="top_file_panel", classes="components file_panel")
#
#             yield SelectionList[str](
#                     *[
#                       #  Selection(tag, tag, tag)
#                       #  for tag in self.tagvals
#                     ],
#                     id="tag_selection", classes="components"
#                 )
#
#             # yield SelectionList[str](
#             #         *[
...
#             #         ],
#             #         id="images_to_use_selection", classes="components",
#             #     )
#             #    id="images_to_use",
...
#                 )
#
#     def on_atlas_file_panel_changed(self, message):
#         import re
#
#         def extract_atlas_part(template_path, file_path):
#             # Create a regex pattern dynamically based on the template
#             pattern = re.escape(template_path).replace(r'\{atlas\}', r'(?P<atlas>.+)')
#             #
...
#
#         print('iiiiiiiiiiiiiis it changed???? ', message.value)
#       #  filepaths = ctx.database.get(**self.filters)
#       #  tagvals = ctx.database.tagvalset(self.entity, filepaths=filepaths)
#      #   print('tagvalstagvalstagvalstagvals2222', tagvals)
#         print('self.app.walk_children(AtlasFilePanel)---------self.app.walk_children(AtlasFilePanel)', self.app.walk_children(AtlasFilePanel))
#
#         tagvals = set([])
...
#
#         tagvals = tagvals | set([extract_atlas_part(template_path, file_path) for file_path in message.value['files']])
#
#         if self.app.walk_children(AtlasFilePanel) != []:
#             for w in self.walk_children(AtlasFilePanel)[0].walk_children(FileItem):
#                 print('--------------w.pattern_match_results-----------', w.pattern_match_results)
#                 template_path = w.pattern_match_results['file_pattern']
#                 if isinstance(template_path, Text):
#                     template_path = template_path.plain
#                     tagvals = tagvals | set([extract_atlas_part(template_path, file_path) for file_path in w.pattern_match_results['files']])
#                     print('*/*/***//', set([extract_atlas_part(template_path, file_path) for file_path in w.pattern_match_results['files'] ]))
...
#        self.get_widget_by_id("tag_selection").border_title = "Atlas files"
...
#
#        self.get_widget_by_id("top_file_panel").border_title = "Atlas seed images"
...
#         self.feature_dict["atlases"] = selection_list.control.selected
#
