...
def load_data(
    ...
    # Update the masks
    mask_data = np.logical_and(mask_data, np.isfinite(cope_data))
    if var_cope_files is not None:
        mask_data = np.logical_and(mask_data, np.isfinite(var_cope_data))

    # Zero out voxels that are not in the mask
    cope_data[np.logical_not(mask_data)] = np.nan
    var_cope_data[np.logical_not(mask_data)] = np.nan
